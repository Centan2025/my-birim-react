...
async function main() {
  ...
  console.log(
    'Patching published AND draft homePage documents with 100% Full-Screen Cinematic Flow...'
  )
  const homeDocs = await client.fetch(`*[_type == "homePage"]{ _id }`)
  for (const doc of homeDocs) {
    console.log(`Updating ${doc._id}...`)
    await client.patch(doc._id).set({contentBlocks: fullScreenBlocks}).commit()
  }
