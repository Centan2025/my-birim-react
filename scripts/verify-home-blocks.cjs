...
async function verify() {
  ...
  home?.contentBlocks?.forEach((b, idx) => {
    console.log(
      `[Block ${idx}] Key: ${b._key} | Order: ${b.order} | Type: ${b.mediaType} | Pos: ${b.position} | Title: ${JSON.stringify(b.title?.tr || b.title)}`
    )
    if (b.linkText)
      console.log(`   CTA: ${JSON.stringify(b.linkText.tr || b.linkText)} -> ${b.linkUrl}`)
  })
}
