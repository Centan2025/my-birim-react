...
const migrateProducts = async () => {
  ...
  for (const product of products) {
    ...
    // Main Images
    const mainImageR2 = await processImageField(
      product._id,
      'mainImage',
      product.mainImage,
      product.mainImageR2
    ...
    if (mainImageR2) {
      patch.set({mainImageR2})
      hasChanges = true
    }

    const mobileR2 = await processImageField(
      product._id,
      'mainImageMobile',
      product.mainImageMobile,
      product.mainImageMobileR2
    )
    if (mobileR2) {
      patch.set({mainImageMobileR2: mobileR2})
      hasChanges = true
    }

    const desktopR2 = await processImageField(
      product._id,
      'mainImageDesktop',
      product.mainImageDesktop,
      product.mainImageDesktopR2
    )
    if (desktopR2) {
      patch.set({mainImageDesktopR2: desktopR2})
      hasChanges = true
    }
    ...
    if (product.alternativeMedia && Array.isArray(product.alternativeMedia)) {
      ...
      for (let i = 0; i < newMedia.length; i++) {
        ...
        if (item.type === 'image') {
          const imgR2 = await processImageField(
            product._id,
            `alternativeMedia_${i}_image`,
            item.image,
            item.imageR2
          )
          if (imgR2) {
          ...
          const mobR2 = await processImageField(
            product._id,
            `alternativeMedia_${i}_mobile`,
            item.imageMobile,
            item.imageMobileR2
          )
          if (mobR2) {
            item.imageMobileR2 = mobR2
            arrayChanged = true
          }

          const deskR2 = await processImageField(
            product._id,
            `alternativeMedia_${i}_desktop`,
            item.imageDesktop,
            item.imageDesktopR2
          )
          if (deskR2) {
            item.imageDesktopR2 = deskR2
            arrayChanged = true
          }
        }
      }
      if (arrayChanged) {
        patch.set({alternativeMedia: newMedia})
        hasChanges = true
      }
    }
    ...
    if (product.dimensionImages && Array.isArray(product.dimensionImages)) {
      ...
      if (arrayChanged) {
        patch.set({dimensionImages: newDims})
        hasChanges = true
      }
    }
...
const migrateHomePage = async () => {
  ...
  if (doc.inspirationSection) {
    ...
    const bgR2 = await processImageField(
      doc._id,
      'insp_bg',
      insp.backgroundImage,
      insp.backgroundImageR2
    )
    if (bgR2) {
      insp.backgroundImageR2 = bgR2
      inspChanged = true
    }
    ...
    const mobR2 = await processImageField(
      doc._id,
      'insp_bg_mob',
      insp.backgroundImageMobile,
      insp.backgroundImageMobileR2
    )
    if (mobR2) {
      insp.backgroundImageMobileR2 = mobR2
      inspChanged = true
    }
    ...
    const deskR2 = await processImageField(
      doc._id,
      'insp_bg_desk',
      insp.backgroundImageDesktop,
      insp.backgroundImageDesktopR2
    )
    if (deskR2) {
      insp.backgroundImageDesktopR2 = deskR2
      inspChanged = true
    }

    if (inspChanged) {
      patch.set({inspirationSection: insp})
      changed = true
    }
  }
