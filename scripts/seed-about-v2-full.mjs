...
async function seedFullAboutV2() {
  console.log('🚀 Hakkımızda (Klasik) verileri okunup Hakkımızda V2’ye aktarılıyor...')

  const originalDoc = await client.fetch('*[_type == "aboutPage"][0]')

  const heroImageR2 = originalDoc?.heroImageR2 || originalDoc?.historySection?.imageR2
  const historyImageR2 = originalDoc?.historySection?.imageR2
  const identityImageR2 = originalDoc?.identitySection?.imageR2
  const qualityImageR2 = originalDoc?.qualitySection?.imageR2

  const historyMedia = originalDoc?.historySection?.media || []
  const identityMedia = originalDoc?.identitySection?.media || []
  const qualityMedia = originalDoc?.qualitySection?.media || []
    ...
    eras: [
      {
        ...
        ...(historyImageR2 && {imageR2: historyImageR2}),
      },
      {
        _key: 'era-1992',
        _type: 'eraItem',
        ...
        ...(qualityImageR2 && {imageR2: qualityImageR2}),
      },
      {
        _key: 'era-2008',
        _type: 'eraItem',
        ...
        ...(identityImageR2 && {imageR2: identityImageR2}),
      },
      {
        _key: 'era-today',
        _type: 'eraItem',
        ...
        ...(heroImageR2 && {imageR2: heroImageR2}),
      },
    ],
    historySection: {
      title: {
  ...
  try {
    ...
    console.log('✅ Tüm görseller ile Hakkımızda V2 dokümanı Sanity CMS’e yüklendi! ID:', res._id)
  } catch (err) {
