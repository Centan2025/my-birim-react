...
async function syncFactoryPage() {
  console.log('🚀 Fabrika V2 içerikleri Sanity CMS veritabanına aktarılıyor...')

  const existingDoc = await client.fetch('*[_type == "factoryPage" && !(_id in path("drafts.**"))][0]')
  if (!existingDoc) {
    console.error('❌ factoryPage dokümanı bulunamadı!')
    return
  }

  const gallery = existingDoc.gallery || []
  console.log(`📸 Mevcut galeride ${gallery.length} görsel tespit edildi.`)
  ...
  // 1. Published dokümanı güncelle
  console.log(`📝 Published doküman güncelleniyor: ${existingDoc._id}`)
  await client.patch(existingDoc._id).set(patchData).commit()
  console.log('✅ Published doküman başarıyla güncellendi!')

  // 2. Draft doküman varsa onu da güncelle
  const draftId = `drafts.${existingDoc._id}`
  const draftDoc = await client.getDocument(draftId)
  if (draftDoc) {
    console.log(`📝 Draft doküman güncelleniyor: ${draftId}`)
    await client.patch(draftId).set(patchData).commit()
    console.log('✅ Draft doküman başarıyla güncellendi!')
  }

  console.log('🎉 Fabrika V2 tüm metinleri ve görselleri Sanity CMS’e eksiksiz kaydedildi!')
}

syncFactoryPage().catch(err => {
