...
app.post('/api/inquiry', async (req, res) => {
  ...
  // 1. Supabase inquiries tablosuna kaydet
  if (supabaseAdmin) {
    try {
      ...
      console.log(`📋 [Inquiry] Yeni talep Supabase'e kaydedildi: ${name} (${email})`)
    } catch (dbErr) {
      console.warn('[Inquiry] Supabase kayıt uyarısı:', dbErr.message || dbErr)
    }
  }

  // 2. E-posta içeriği oluştur
  ...
  try {
    if (resendClient) {
      ...
      console.log(`📧 [Inquiry] SMTP ile teklif bildirimi gönderildi: ${email}`)
    } else {
      console.log(`📧 [SIMÜLASYON] Teklif talebi maili gönderilecekti → ${email}`)
    }
  } catch (mErr) {
    console.warn('[Inquiry] Mail gönderim uyarısı:', mErr.message || mErr)
  }

  return res.json({ok: true, message: 'Inquiry processed'})
})
