...
async function seed() {
  ...
  const products = await client.fetch(`*[_type == "product" && !(_id in path("drafts.**"))][0..5]{ _id, name }`);
  console.log(`Found ${products.length} real products in Sanity:`, products.map(p => p.name?.tr || p._id));
