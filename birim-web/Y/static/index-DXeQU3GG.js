import {t as z, f as x} from './sanity.config-BN7nEAqm.js'
import 'sanity'
...
class N {
  ...
  async serializeEventStream({eventStream: i, requestSchema: a, initialRequest: o}) {
    const m = this.marshaller,
      d = a.getEventStreamMember(),
      S = a.getMemberSchema(d),
      y = this.serializer,
    ...
    return m.serialize(p, (r) => {
      if (r[h]) return {headers: r.headers, body: r.body}
      const t = Object.keys(r).find((s) => s !== '__type') ?? '',
        {
          additionalHeaders: e,
          body: n,
          eventType: l,
  ...
  async deserializeEventStream({response: i, responseSchema: a, initialResponseContainer: o}) {
    const m = this.marshaller,
      d = a.getEventStreamMember(),
      y = a.getMemberSchema(d).getMemberSchemas(),
      f = Symbol('initialResponseMarker'),
      h = m.deserialize(i.body, async (t) => {
            ...
            for (const [s, g] of l.structIterator()) {
              ...
              if (((u = u || !!(v || w)), w))
                g.isBlobSchema()
                  ? (c[s] = n)
                  : g.isStringSchema()
                    ? (c[s] = (this.serdeContext?.utf8Encoder ?? z)(n))
                    : g.isStructSchema() && (c[s] = await this.deserializer.read(g, n))
              else if (v) {
                const b = t[e].headers[s]?.value
                b != null &&
                  (g.isNumericSchema()
                    ? b && typeof b == 'object' && 'bytes' in b
                      ? (c[s] = BigInt(b.toString()))
                      : (c[s] = Number(b))
                    : (c[s] = b))
              }
            }
  ...
  writeEventBody(i, a, o) {
    const m = this.serializer
    let d = i,
      S = null,
      y
    const f = a.getSchema()[4].includes(i),
      h = {}
    if (f) {
      const t = a.getMemberSchema(i)
      if (t.isStructSchema()) {
          ...
          if (c) S = e
            ...
            ;(n.isNumericSchema()
              ...
              : n.isTimestampSchema()
                ...
                : n.isStringSchema()
                  ? (s = 'string')
                  : n.isBooleanSchema() && (s = 'boolean'),
              u != null && ((h[e] = {type: s, value: u}), delete o[i][e]))
          }
        }
        if (S !== null) {
