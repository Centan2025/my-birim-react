...
// R2 Upload Helper
const uploadToR2 = async (file: File, path: string): Promise<{ url: string; width?: number; height?: number } | null> => {
  try {
    // Sıkıştır (sadece görsel ise)
    const processedFile = isImageFile(file.name) ? await compressImage(file) : file

    const arrayBuffer = await processedFile.arrayBuffer()
    const buffer = new Uint8Array(arrayBuffer)

    // WebP uzantısını ekle eğer sıkıştırıldıysa
    let fileName = file.name
    if (isImageFile(file.name) && !file.name.toLowerCase().endsWith('.webp')) {
    ...
    await r2Client.send(
      new PutObjectCommand({
        ...
        Key: key,
        Body: buffer,
        ContentType: isImageFile(file.name) ? 'image/webp' : file.type,
      })
    )

    // Görsel boyutlarını al (opsiyonel)
    let dimensions = {}
    ...
    if (cmsMediaHashes.has(hash)) {
      ...
      if ((isImg && (!existingItem.imageR2 || existingItem.image)) ||
        ...
        if (r2Url) {
          ...
          if (isVideo) {
            updatedItem.videoFileR2 = { _type: 'r2Asset', url: r2Url.url }
        ...
        if (existing && ((mainFile.isVideo && existing.videoFileR2 && !existing.videoFile) || (!mainFile.isVideo && existing.imageR2 && !existing.image))) {
          ...
          if (mobileFile) {
            if (mainFile.isVideo) {
              ...
              if (r2Url) {
                panelItem.videoFileMobileR2 = { _type: 'r2Asset', url: r2Url.url }
              }
            } else {
          ...
          if (desktopFile) {
            if (mainFile.isVideo) {
              ...
              if (r2Url) {
                panelItem.videoFileDesktopR2 = { _type: 'r2Asset', url: r2Url.url }
              }
            } else {
      ...
      if (existing && existing.fileR2 && !existing.file) {
        ...
        if (r2Url) {
          ...
          newDrawings.push({
            ...
            fileR2: { _type: 'r2Asset', url: r2Url.url }
          })
          hasExclusiveChanges = true
        } else if (existing) {
          newDrawings.push(existing)
  ...
  for (const [hash, mediaInfo] of folderMediaMap.entries()) {
    ...
    if (existing && ((isVideo && existing.videoFileR2 && !existing.videoFile) || (!isVideo && existing.imageR2 && !existing.image))) {
      ...
      if (r2Url) {
        ...
        if (isVideo) {
          item.videoFileR2 = { _type: 'r2Asset', url: r2Url.url }
        } else {
