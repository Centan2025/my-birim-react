        ...
        <div className="w-full max-w-[95%] md:max-w-[92%] lg:max-w-[80vw] mx-auto px-4 md:px-8 lg:px-0 py-6 md:py-8 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0">
          ...
          <div className="flex-shrink-0 md:ml-4 w-full md:w-auto flex justify-center md:justify-end">
            <motion.div
              ...
              whileTap={{scale: 0.96}}
              className={`inline-flex items-center justify-center gap-2.5 font-inter border transition-colors duration-300 shadow-sm rounded-none overflow-hidden cursor-pointer ${
                isExpanded
                  ? 'bg-white/10 border-white text-white px-5 py-2.5 md:px-6 md:py-3'
                  : 'bg-transparent border-gray-400 text-white px-6 py-2.5 md:px-7 md:py-3 hover:bg-white/10 hover:border-white'
              }`}
            >
              {/* Sade ve Akıcı Metin Dönüşümü (Clean Roll-Over Text Morph) */}
              <div className="relative overflow-hidden h-5 flex items-center justify-center">
                <AnimatePresence mode="popLayout" initial={false}>
                  <motion.span
                    key={isExpanded ? 'close' : 'join'}
                    initial={{y: isExpanded ? 18 : -18, opacity: 0}}
                    animate={{y: 0, opacity: 1}}
                    exit={{y: isExpanded ? -18 : 18, opacity: 0}}
                    transition={{duration: 0.28, ease: [0.16, 1, 0.3, 1]}}
                    className="text-[10px] md:text-[11px] uppercase tracking-[0.15em] font-medium whitespace-nowrap block"
                  >
              ...
              {/* İkon Dönüşümü */}
              <div className="relative overflow-hidden w-3.5 h-3.5 flex items-center justify-center flex-shrink-0">
                <AnimatePresence mode="popLayout" initial={false}>
                  <motion.span
                    key={isExpanded ? 'x' : 'chevron'}
                    initial={{y: isExpanded ? 14 : -14, opacity: 0, rotate: isExpanded ? -90 : 90}}
                    animate={{y: 0, opacity: 1, rotate: 0}}
                    exit={{y: isExpanded ? -14 : 14, opacity: 0, rotate: isExpanded ? 90 : -90}}
                    transition={{duration: 0.28, ease: [0.16, 1, 0.3, 1]}}
        ...
        {isExpanded && (
          ...
          >
            <div
              ...
              style={{background: 'linear-gradient(135deg, #ffffff 0%, #f4f4f4 50%, #e2e2e2 100%)'}}
            >
