...
export interface DesignersVersionSwitcherProps {
  activeVersion: 'v1' | 'v2'
  onChange: (version: 'v1' | 'v2') => void
}

export const DesignersVersionSwitcher: React.FC<DesignersVersionSwitcherProps> = ({
  activeVersion,
  onChange,
}) => {
  ...
  const versions: {id: 'v1' | 'v2'; label: string; descTr: string; descEn: string}[] = [
    {id: 'v1', label: 'V1', descTr: 'Izgara', descEn: 'Grid'},
    {id: 'v2', label: 'V2', descTr: 'Tam Ekran', descEn: 'Fullscreen'},
  ]

  return (
    <aside
      aria-label={isTr ? 'Tasarımcılar Görünüm Seçici' : 'Designers View Switcher'}
        ...
        {versions.map(v => {
          ...
          return (
            ...
            >
              {isActive && (
                <motion.div
                  layoutId="designers-version-pill"
...
export default DesignersVersionSwitcher
