...
export const HomeHero: React.FC<HomeHeroProps> = ({ content }) => {
  ...
  const arrowInLeft: CSSProperties = {
    transform: 'scale(0)',
    opacity: 0,
    animation: 'home-button-grow 600ms cubic-bezier(0.34, 1.56, 0.64, 1) 120ms forwards',
  }
  const arrowInRight: CSSProperties = {
    transform: 'scale(0)',
    opacity: 0,
    animation: 'home-button-grow 600ms cubic-bezier(0.34, 1.56, 0.64, 1) 200ms forwards',
  }
            ...
            const realIndex =
              slideCount > 1 && (index === 0 || index === totalSlides - 1)
                ? index === 0
                  ...
                  : 0
                : index - 1
            const isInitialSlide = slideCount <= 1 ? index === 0 : index === 1 // klonlu dizide: [son, ...heroMedia, ilk] → ilk gerçek slide index 1
                  ...
                  return (
                    <button
                      ...
                      className={`relative h-2 rounded-none transition-all duration-500 ease-in-out group ${areDotsVisible ? 'animate-dot-grow' : 'opacity-0 scale-0'
                        } ${isActive ? 'w-12 bg-red-700' : 'w-2 bg-white/40 hover:bg-white/60'}`}
                      style={{
                    ...
                    >
                      {isActive && (
                        <div
                          ...
                          className="absolute top-0 left-0 h-full rounded-none bg-red-700 animate-fill-line"
                        ></div>
                      )}
                    </button>
                  )
