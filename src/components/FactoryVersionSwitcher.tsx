...
interface FactoryVersionSwitcherProps {
  activeVersion: 'v1' | 'v2' | 'v3'
  onChange: (version: 'v1' | 'v2' | 'v3') => void
}

export function FactoryVersionSwitcher({activeVersion, onChange}: FactoryVersionSwitcherProps) {
  const {locale} = useTranslation()
  const isTr = locale === 'tr'

  const versions: {id: 'v1' | 'v2' | 'v3'; label: string; descTr: string; descEn: string}[] = [
    {id: 'v1', label: 'V1', descTr: 'Klasik', descEn: 'Classic'},
    {id: 'v2', label: 'V2', descTr: 'Sinematik', descEn: 'Cinematic'},
    {id: 'v3', label: 'V3', descTr: 'Blueprint', descEn: 'Blueprint'},
  ]

  return (
    <aside
      aria-label={isTr ? 'Sayfa Sürüm Seçici' : 'Page Version Switcher'}
