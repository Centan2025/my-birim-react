...
export const HeaderMobileMenuOverlay: FC<HeaderMobileMenuOverlayProps> = props => {
  ...
  return (
    ...
    <div
      ...
      id="mobile-menu"
      className={`mobile-menu-overlay fixed inset-0 lg:hidden z-40 block overflow-y-auto overscroll-contain text-white pb-8 px-6 transition-all duration-400 ease-[cubic-bezier(0.76,0,0.24,1)] ${
        isMobileMenuOpen
              ...
              {supportedLocales.map(langCode => {
                ...
                return (
                  <button
                    ...
                    className={`group relative px-2.5 py-1.5 text-sm tracking-[0.08em] transition-all duration-200 ${
                      isActive ? 'text-white' : 'text-gray-400/90 hover:text-white'
      ...
      <nav className="flex flex-col justify-start gap-6">
        ...
        <div>
          <button
            ...
            className={`group flex items-center justify-between w-full text-2xl md:text-3xl leading-tight text-white transition-all duration-400 ${
              isMobileMenuOpen ? 'translate-x-0 opacity-100' : '-translate-x-10 opacity-0'
            }`}
          >
            <span>
              <CrossFadeText
                text={(t('products') || '').toLocaleUpperCase(locale === 'tr' ? 'tr-TR' : 'en-US')}
                triggerKey={locale}
            ...
            <div className="pl-6 flex flex-col gap-3">
              {categories.map((category, index) => (
                <NavLink
                  ...
                  className={`group flex items-center justify-between text-2xl md:text-3xl leading-tight text-gray-300 hover:text-white transition-all duration-400 ${
                    isMobileMenuOpen ? 'translate-x-0 opacity-100' : '-translate-x-10 opacity-0'
                  }`}
                  onClick={() => {
                    onCloseAll()
              ...
              <NavLink
                ...
                className={`group flex items-center justify-between text-2xl md:text-3xl leading-tight text-gray-300 hover:text-white transition-all duration-400 border-t border-white/20 pt-3 mt-2 ${
                  isMobileMenuOpen ? 'translate-x-0 opacity-100' : '-translate-x-10 opacity-0'
                }`}
                onClick={() => {
                  onCloseAll()
