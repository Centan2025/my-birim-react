...
export const R2_DOMAIN = import.meta.env['VITE_R2_DOMAIN'] || 'https://birim-assets.web-birim.workers.dev'
export const R2_ORIGIN_DOMAIN = import.meta.env['VITE_R2_ORIGIN_DOMAIN'] || 'https://pub-5e705b2a702d4bb1a3631c558917599d.r2.dev'

const defaultEnableFallback = import.meta.env.PROD ? 'false' : 'true'
export const ENABLE_LOCAL_FALLBACK = String((import.meta as any).env?.VITE_ENABLE_LOCAL_FALLBACK ?? defaultEnableFallback).toLowerCase() !== 'false'
...
export const rewriteR2Url = (url: string | undefined, hasResponsiveSizes?: boolean): string => {
    ...
    if (R2_ORIGIN_DOMAIN && R2_DOMAIN && R2_ORIGIN_DOMAIN !== R2_DOMAIN && result.startsWith(R2_ORIGIN_DOMAIN)) {
...
export const mapImage = (
    ...
    const hasBuilderMeta = (img as any)?.crop || (img as any)?.hotspot || (img as any)?.asset?._ref || (img as any)?.asset?._id
...
export const mapMediaUrl = (
    ...
    if (type === 'image') {
        ...
        if (r2Url) {
            if (r2Url.startsWith('migration/') && R2_DOMAIN) return `${R2_DOMAIN}/${r2Url}`.replace(/ /g, '%20')
            return rewriteR2Url(r2Url)
        }
        if ((isMobile || isDesktop) && m?.videoFileR2?.url) {
            const genericR2 = m.videoFileR2.url
            if (genericR2.startsWith('migration/') && R2_DOMAIN) return `${R2_DOMAIN}/${genericR2}`.replace(/ /g, '%20')
