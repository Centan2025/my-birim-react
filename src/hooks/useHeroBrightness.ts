...
export function useHeroBrightness(isMobile: boolean, pathname: string, contextBrightness: number | null) {
    ...
    useEffect(() => {
        ...
        const checkTopImageBrightness = () => {
            if (pathname !== pathname) { // This is a bit redundant but matches the original check logic
                isCancelled = true
                return
            }
