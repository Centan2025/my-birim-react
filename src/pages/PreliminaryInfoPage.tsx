...
import {getPreliminaryInfoForm} from '../services/cms'
import {useTranslation} from '../i18n'
import PortableTextLite from '../components/PortableTextLite'
import type {PreliminaryInfoForm} from '../types'
...
export default function PreliminaryInfoPage() {
  const [form, setForm] = useState<PreliminaryInfoForm | null>(null)
  const {t, locale} = useTranslation()
  const localizedContent = form?.content as Record<string, unknown> | undefined
  ...
  const title = form?.title ? t(form.title) : 'Ön Bilgilendirme Formu'
  const contentBlocks =
  ...
  useSEO({
    title: `BIRIM - ${title}`,
    description:
      t('preliminary_info_description') ||
  ...
  const updated = form?.updatedAt
    ? new Date(form.updatedAt).toLocaleDateString(locale === 'en' ? 'en-GB' : 'tr-TR')
