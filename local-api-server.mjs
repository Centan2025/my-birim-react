...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        if (user.userType === 'email_subscriber') return res.status(403).json({ error: 'Bu sadece abonelik kaydı, lütfen tam üyelik alın.' })
...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        ...
        if (existingUser) {
            if (existingUser.userType === 'email_subscriber') {
                ...
                const updatedUser = await sanityClient
                    ...
                    .set({ password: passwordHash, name: name || '', company: company || '', profession: profession || '', country: country || existingUser.country || '', userType: 'full_member', isVerified: false, verificationToken: crypto.randomUUID() })
                    .commit()
                return res.status(200).json({ success: true, message: 'Abonelik hesabınız tam üyeliğe yükseltildi.', user: { id: updatedUser._id, email: updatedUser.email, userType: 'full_member' } })
            }
        ...
        const newUser = await sanityClient.create({
            _type: 'user', email: normEmail, password: passwordHash, name: name || '', company: company || '', profession: profession || '', country: country || '', userType: 'full_member', isActive: true, isVerified: false, verificationToken, createdAt: new Date().toISOString(),
        })
        return res.status(201).json({ success: true, user: { id: newUser._id, email: newUser.email, name: newUser.name, verificationToken: newUser.verificationToken } })
    } catch (err) {
...
app.post('/api/auth/verify', async (req, res) => {
    ...
    try {
        const user = await sanityClient.fetch(`*[_type == "user" && verificationToken == $token][0]`, { token })
        if (!user) return res.status(400).json({ error: 'Geçersiz veya süresi dolmuş token.' })
        if (user.isVerified) return res.status(200).json({ success: true, message: 'E-posta zaten doğrulanmış.' })
...
app.post('/api/auth/subscribe', async (req, res) => {
    ...
    const normEmail = email.trim().toLowerCase()
    try {
        const existing = await sanityClient.fetch(`*[_type == "user" && lower(email) == $email && !defined(_deleted)][0]`, { email: normEmail })
        if (existing) return res.status(400).json({ error: 'Bu e-posta adresi zaten kayıtlı.' })
        const newUser = await sanityClient.create({ _type: 'user', email: normEmail, name: '', company: '', profession: '', userType: 'email_subscriber', isActive: true, createdAt: new Date().toISOString() })
...
app.post('/api/auth/reset-request', async (req, res) => {
    ...
    const normEmail = email.trim().toLowerCase()
    try {
        const user = await sanityClient.fetch(`*[_type == "user" && lower(email) == $email && !defined(_deleted)][0]`, { email: normEmail })
        if (!user) return res.status(404).json({ error: 'Kullanıcı bulunamadı.' })
        const resetToken = crypto.randomUUID()
        const resetPasswordExpires = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString()
        await sanityClient.patch(user._id).set({ resetPasswordToken: resetToken, resetPasswordExpires }).commit()
...
app.post('/api/auth/reset-password', async (req, res) => {
    ...
    if (!token || !newPassword) return res.status(400).json({ error: 'Token ve yeni şifre gereklidir.' })
    try {
        const user = await sanityClient.fetch(`*[_type == "user" && resetPasswordToken == $token && resetPasswordExpires > $now][0]`, { token, now: new Date().toISOString() })
        if (!user) return res.status(400).json({ error: 'Geçersiz veya süresi dolmuş token.' })
        const passwordHash = await bcrypt.hash(newPassword, 10)
        await sanityClient.patch(user._id).set({ password: passwordHash }).unset(['resetPasswordToken', 'resetPasswordExpires']).commit()
...
app.listen(PORT, () => {
    ...
    console.log(`   SANITY_TOKEN: ${SANITY_TOKEN ? '✓ Yüklendi' : '✗ YOK! (.env.local\'e SANITY_TOKEN ekle)'}`)
