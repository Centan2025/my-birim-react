                ...
                <div className="flex flex-col h-full">
                    ...
                    {cartItems.length > 0 ? (
                        <>
                                    ...
                                    <div key={item.product.id} className="flex items-start gap-4">
                                        ...
                                        <div className="flex-grow">
                                            ...
                                            <div className="flex items-center gap-2 mt-2">
                                                <button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="p-1 border rounded-md hover:bg-gray-100"><MinusIcon /></button>
                                                <span className="w-8 text-center">{item.quantity}</span>
                                                <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} className="p-1 border rounded-md hover:bg-gray-100"><PlusIcon /></button>
                                            </div>
                                        </div>
                                        <button onClick={() => removeFromCart(item.product.id)} className="text-gray-400 hover:text-red-500 text-sm">Remove</button>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="p-6 border-t">
                                <div className="flex justify-between items-center font-semibold text-lg text-gray-800">
                                    <span>Subtotal</span>
                                    <span>{new Intl.NumberFormat(locale, { style: 'currency', currency: 'TRY' }).format(cartTotal)}</span>
                                </div>
                        ...
                        <div className="flex-grow flex flex-col items-center justify-center p-6 text-center">
                            ...
                            <button onClick={toggleCart} className="mt-4 text-gray-800 font-semibold hover:underline">Continue Shopping</button>
                        </div>
                    )}
                </div>
            </div>
